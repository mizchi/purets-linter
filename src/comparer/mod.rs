...
pub use metrics::{CodeMetrics, MetricChanges, MetricsComparison};
...
pub fn print_summary(comparisons: &[MetricsComparison]) {
    ...
    if total_changes.code_lines_change < 0 {
        println!(
            ...
            "✓".green(),
            -total_changes.code_lines_change,
            ((-total_changes.code_lines_change as f64 / total_before.code_lines as f64) * 100.0)
                as i32
        );
    }
    ...
    if total_changes.branch_count_change < 0 {
        println!(
            "  {} Complexity reduced by {} branches",
            "✓".green(),
            -total_changes.branch_count_change
        );
    }
