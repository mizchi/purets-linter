...
impl TsConfigValidator {
    ...
    pub fn validate(&mut self) -> Result<()> {
        ...
        if !path.exists() {
            self.errors.push(format!("tsconfig.json not found at {}", tsconfig_path));
            return Ok(());
        }
        ...
        let _raw_json: Value = serde_json::from_str(&content)
            .context("Invalid JSON in tsconfig.json")?;
        
        // Parse into struct
        let tsconfig: TsConfig = serde_json::from_str(&content)
            .context("Failed to parse tsconfig.json structure")?;
    ...
    fn validate_compiler_options(&mut self, options: &Option<CompilerOptions>) {
        match options {
            None => {
                self.errors.push("compilerOptions is missing in tsconfig.json".to_string());
            }
            Some(opts) => {
                // Check strict mode
                ...
                if opts.strict != Some(true) {
                    let strict_options = vec![
                        ...
                        (opts.strict_property_initialization, "strictPropertyInitialization"),
                    ];
                ...
                if opts.no_implicit_returns != Some(true) {
                    self.warnings.push("Consider enabling noImplicitReturns for safer code".to_string());
                }
                
                if opts.no_fallthrough_cases_in_switch != Some(true) {
                    self.warnings.push("Consider enabling noFallthroughCasesInSwitch".to_string());
                }
                
                if opts.no_unused_locals != Some(true) {
                    self.warnings.push("Consider enabling noUnusedLocals".to_string());
                }
                
                if opts.no_unused_parameters != Some(true) {
                    self.errors.push("noUnusedParameters must be set to true".to_string());
                }
                
                if opts.exact_optional_property_types != Some(true) {
                    self.warnings.push("Consider enabling exactOptionalPropertyTypes for stricter typing".to_string());
                }
                
                if opts.no_unchecked_indexed_access != Some(true) {
                    self.warnings.push("Consider enabling noUncheckedIndexedAccess for safer array/object access".to_string());
                }
                
                // Check for problematic settings
                if opts.allow_unreachable_code == Some(true) {
                    self.errors.push("allowUnreachableCode should not be true".to_string());
                }
                
                if opts.allow_unused_labels == Some(true) {
                    self.errors.push("allowUnusedLabels should not be true".to_string());
                }
                
                // Check required settings for .ts extension imports
                if opts.allow_import_ts_extension != Some(true) {
                    self.errors.push("allowImportTsExtension must be set to true".to_string());
                }
                
                if opts.verbatim_module_syntax != Some(true) {
                    self.errors.push("verbatimModuleSyntax must be set to true".to_string());
                }
                
                // Check module and target
                if let Some(module) = &opts.module {
                    if module != "ESNext" && module != "ES2022" && module != "ES2020" {
                        self.warnings.push(format!("Consider using ESNext or ES2022 for module, currently: {}", module));
                    }
                }
                
                if let Some(target) = &opts.target {
                    if target != "ESNext" && target != "ES2022" && target != "ES2020" {
                        self.warnings.push(format!("Consider using ESNext or ES2022 for target, currently: {}", target));
                    }
                }
            }
        }
